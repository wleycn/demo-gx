...
- A single change must not exceed **40%** of the file total. The threshold applies **only to files that reach 200 lines after the change**; below that the file is not limited. If it exceeds the cap, split it into several changes and verify step by step.
