...
- A commit message states what changed and why, **with no author-type marker**: an agent acts on the user’s behalf, so its commit is the user’s commit and carries no `[AI]` tag. The machine gate warns about a changed code file without the header. The rule therefore does not rest on anyone remembering it.
- A single change must not exceed **40%** of the file total. The threshold applies **only to files that reach 200 lines before the change**; below that the file is not limited. If it exceeds the cap, split it into several changes and verify step by step.
