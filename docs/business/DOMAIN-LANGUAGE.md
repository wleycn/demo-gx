...
| quarantine | quarantine | The act of isolating invalid records into `errors/quarantine/` instead of blocking the pipeline. Invalid records get an error envelope and the pipeline continues processing valid data. | DATA-DESIGN, INTERFACE-DESIGN, schema_validator.py |
...
| strict mode | strict mode | Validation policy that rejects unknown fields not declared in `config/contract/schema.yaml`. Row-level key presence is judged from the preserved `_raw_json` when available. | MODULE-DESIGN, schema_validator.py |
