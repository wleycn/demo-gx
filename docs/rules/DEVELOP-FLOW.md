...
- The threshold applies **only to files that reach 200 lines before the change**; below that the file is not limited (a small file is cheap to review even when rewritten whole). The line count is measured on the file **before** the change; a new file is not limited
